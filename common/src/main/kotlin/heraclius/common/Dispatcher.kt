...
class Dispatcher {
    // 存储事件与其对应发射器的映射
    private val _evtMapEmitter = mutableMapOf<Symbols.Symbol<*>, Emitter<*>>()
     ...
     * @return 与该事件关联的发射器
     */
    fun <T> emitter(evt: Symbols.Symbol<T>): Emitter<T> {
     ...
     * @param data 需要发送的数据
     */
    fun <T> dispatch(evt: Symbols.Symbol<T>, data: T) {
        @Suppress("UNCHECKED_CAST")
     ...
     */
    fun hasEvent(evt: Symbols.Symbol<*>): Boolean {
