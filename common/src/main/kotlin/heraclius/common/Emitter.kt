...
// 触发器的封装数据
data class EmitterData<T>(val listener: Listener<T>, val once: Boolean)
...
class Emitter<T> {
    // 存储所有注册的监听器数据
    private val dataArray = mutableListOf<EmitterData<T>>()
    ...
    fun on(listener: Listener<T>, once: Boolean = false): Emitter<T> {
        dataArray.add(EmitterData(listener, once))
        return this
    }
